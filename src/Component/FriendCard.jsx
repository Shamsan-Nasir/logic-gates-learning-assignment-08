    ...
    return (
        <Link to={`/${cardData.id}`}>
            <div className='hovering rounded-lg bg-white shadow flex flex-col justify-center items-center p-6 gap-2 cursor-pointer h-full'>
                ...
                <div className='flex  flex-wrap justify-center gap-1 items-center'>{cardData.tags.map((tag, index) => <div key={index} className='bg-[#CBFADB] px-3 py-2.5 text-xs text-center rounded-full font-medium'>{tag.toUpperCase()}</div>)}</div>
